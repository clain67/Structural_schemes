//g++ ZDST_linear.cpp -O2 -lqd -o ZDST_linear -mavx2
#include<iostream>
#include <iomanip>
#include <fstream>
#include <string>
...
void fun(PB<real> &pb, size_t r)
    ...
    pb.blockT(r)(0)=pb.lambda*pb.blockS(r)(0);
    return;
    }
...
// ------------------ initialization -----------------------
    ...
    for(unsigned int r=0;r<se.R;r++)
        {
        pb.blockZ(r+1)=(1./6.)*Dt3*pb.blockT(r);
    ...
    for(size_t r=1;r<=se.R;r++)
        {
        pb.blockRHS(r)= Dt3*se.bt(r-1)*pb.blockT(0);
    ...
    while (err>ERR_TOL)
        ...
        for(size_t r=1;r<=se.R;r++)
            ...
            for(size_t l=1;l<=se.R;l++)
                {
                pb.blockZ(r)-=Dt3*se.Bt(r-1,l-1)*pb.blockT(l);
...
int main()
    ...
    real t=0,err=0;
    size_t nb_iter=0;
    real T = 1.0,N =10,Dt=T/N;
    std::ofstream file;
    file.open("linear_ZDST.txt");
